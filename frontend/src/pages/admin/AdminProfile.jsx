...
export default function AdminProfile() {
  useDocumentMeta('Admin Profile', 'Manage your admin account.')
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toastError(getErrorMessage(err, 'Could not update password.'))
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <div className="max-w-xl">
      <h1 className="text-2xl font-heading font-bold text-espresso mb-6">Admin Profile</h1>

      <div className="bg-ivory border border-gold/20 rounded-md p-6 mb-6">
        <span className="block text-xs font-semibold uppercase tracking-wide text-warmbrown mb-1">Signed in as</span>
        <p className="text-sm text-charcoal/80">{admin?.email || '—'}</p>
      </div>

      <form onSubmit={handleSubmit} className="bg-ivory border border-gold/20 rounded-md p-6 space-y-5">
        <h2 className="text-sm font-semibold uppercase tracking-wide text-warmbrown">Change Password</h2>
        <div>
